...
int main() {
    std::cout << "=== Fill-Up Grid Trading Strategy Test (BROKER MARKETS) ===" << std::endl;
    std::cout << std::fixed << std::setprecision(2);

    // Configure tick data source - Broker tick data
    TickDataConfig tick_config;
    tick_config.file_path = "validation/Broker/XAUUSD_TICKS_2025.csv";  // Relative to validation directory
    tick_config.format = TickDataFormat::MT5_CSV;
    tick_config.load_all_into_memory = false;  // Streaming mode for large file (4.6 GB)

    // Configure backtest to match MT5 settings - BROKER MARKETS
    ...
    config.leverage = 500.0;       // 1:500 leverage (from Broker settings)
    ...
    try {
        ...
        std::cout << "\n=== Final Results (BROKER MARKETS) ===" << std::endl;
        ...
        std::cout << "\nNow compare with MT5 results in validation/Broker/fill_up/ReportTester-000000.xlsx" << std::endl;
