//+------------------------------------------------------------------+
//| grid_open_upwards_while_going_upwards_optimized.mq5            |
    ...
    if((g_volume_of_open_trades == 0) || (g_checked_last_open_price < current_ask)) {
        ...
        if((g_volume_of_open_trades == 0) || ((current_ask - price_difference) < end_price)) {
            ...
            if(trade_size >= g_min_volume) {
                // Check if at order limit - close smallest profitable position
                if(PositionsTotal() == g_account_limit_orders) {
                    ...
                    if(min_ticket > 0 && m_position.SelectByTicket(min_ticket)) {
                        if(!m_trade.PositionClose(min_ticket)) {
                            ...
                            OnTick(); // Recursive call to open new position
                        }
                    }
                } else {
