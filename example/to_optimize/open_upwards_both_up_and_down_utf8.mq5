...
//|                                                                  |
//+------------------------------------------------------------------+
bool open_up_w_down(double local_unit, int local_direction) {
...
//|                                                                  |
//+------------------------------------------------------------------+
void open_new_up_w_down() {

    if(count_buy == 0) {
        sizing_buy(); }
// open new
//if((count_buy == 0) || (lowest_buy > current_ask + spacing_buy)) {
...
//            trade_size_buy += min_volume_alg; } } }
    ...
    if (count_buy == 0) {
        if (open_up_w_down(trade_size_buy, 1)) {
        ...
        case 0: // constant
            if(lowest_buy > current_ask) {
                //temp = MathFloor((highest_buy - current_ask) / spacing_buy) * trade_size_buy - volume_of_open_trades;
        ...
        case 1: // incremental
            if(lowest_buy > current_ask) {
                ...
                temp5 = MathFloor(temp2 / temp4);
                temp6 = (temp3 + temp5) * min_volume_alg - volume_of_open_trades;
                if (temp6 > 0) {
                    if (open_up_w_down(temp6, 1)) {
                        lowest_buy = current_ask;
                        count_buy++; } } }
...
//double end_price = current_ask * ((100 - (2 * survive)) / 100);
...
//|                                                                  |
//+------------------------------------------------------------------+
double open_up_w_up(double local_unit, double current_ask) {
...
//if((volume_of_open_trades == 0) || (checked_last_open_price < current_ask))
    {
        ...
        if((volume_of_open_trades == 0) || ((current_ask - price_difference) < end_price)) { // if we can't survive anyway, there is no point checking
            ...
            if(trade_size >= min_volume_alg) {
                volume_of_open_trades += open_up_w_up(trade_size, current_ask);
                checked_last_open_price = current_ask; } } }
