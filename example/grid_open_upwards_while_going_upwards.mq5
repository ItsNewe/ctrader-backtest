        ...
        if((volume_of_open_trades == 0) || ((current_ask - price_difference) < end_price)) { // if we can't survive anyway, there is no point checking
            ...
            if(trade_size >= min_volume_alg) {
                ...
                if(PositionsTotal() == AccountInfoInteger(ACCOUNT_LIMIT_ORDERS)) {
                    ...
                    if(m_position.SelectByTicket(min_ticket)) {
                        if(!m_trade.PositionClose(min_ticket)) {
                            Print("Order Close failed, order number: ", min_ticket, " Error: ", GetLastError());
...
//+------------------------------------------------------------------+
int OnInit() {
    m_trade.LogLevel(LOG_LEVEL_ALL); // LOG_LEVEL_ALL LOG_LEVEL_ERRORS LOG_LEVEL_NO
