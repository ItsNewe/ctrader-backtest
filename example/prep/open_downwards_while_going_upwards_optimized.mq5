...
void SizingSell() {
    ...
    if(need_new_grid) {
        ...
        if(margin_level > margin_stop_out_level) {
            // Grid loss is triangular: trade i enters at starting_price + i*spacing
            // and at end_price loses (len - i*spacing). Average loss per unit = len/2.
            // For sizing=1 (incremental), bigger positions are at deeper levels
            // with less distance to lose, so average per unit = len/3.
