...
double g_cost_of_open_trades_upup;  // sum(vol_i * open_price_i) for precise loss calc
    ...
    for(int i = PositionsTotal() - 1; i >= 0; i--) {
        ...
        if(k == 0) {
            if(m_position.SelectByTicket(ticket)) {
                double lots = PositionGetDouble(POSITION_VOLUME);
                double open_price = PositionGetDouble(POSITION_PRICE_OPEN);
                g_volume_of_open_trades_upup += lots;
                g_cost_of_open_trades_upup += lots * open_price;
...
void SizingBuy() {
    ...
    double equity_at_target = equity - g_volume_of_open_trades_fillup * distance * g_contract_size;
    bool need_new_grid = (PositionsTotal() == 0) ||
                         (equity_at_target > g_margin_stop_out_level / 100 * used_margin);
    ...
    if(need_new_grid) {
        ...
        if(margin_level > g_margin_stop_out_level) {
            // BUG FIX: was missing g_spacing_buy multiplier. Each grid level is spaced
            // by g_spacing_buy, so level i loses i*spacing. Sum = spacing * N*(N+1)/2.
            double d_equity = g_contract_size * trade_size * g_spacing_buy * (number_of_trades * (number_of_trades + 1) / 2);
...
void OnTick() {
    ...
    // Up_while_up strategy logic (positions WITHOUT comments)
    {
        ...
        // Scan for up_while_up positions — track both volume and cost basis
        g_volume_of_open_trades_upup = 0;
        g_cost_of_open_trades_upup = 0;
        string result[];
        for(int i = PositionsTotal() - 1; i >= 0; i--) {
            ...
            if(k == 0) {
                ...
                if(m_position.SelectByTicket(ticket)) {
                    double lots = PositionGetDouble(POSITION_VOLUME);
                    g_volume_of_open_trades_upup += lots;
                    g_cost_of_open_trades_upup += lots * PositionGetDouble(POSITION_PRICE_OPEN);
                }
            }
        }
        ...
        double equity_after_existing = equity - g_volume_of_open_trades_upup * distance * g_contract_size;
        bool need_new_trade = (g_volume_of_open_trades_upup == 0) ||
        ...
        if(need_new_trade) {
            ...
            switch(g_calc_mode) {
            case SYMBOL_CALC_MODE_CFDLEVERAGE:
                trade_size = (100 * equity * g_leverage - 100 * g_contract_size * MathAbs(distance) * g_volume_of_open_trades_upup * g_leverage - g_leverage * g_margin_stop_out_level * used_margin) / (g_contract_size * (100 * MathAbs(distance) * g_leverage + 100 * current_spread * g_leverage + starting_price * g_initial_margin_rate * g_margin_stop_out_level));
                break;
            case SYMBOL_CALC_MODE_FOREX:
                trade_size = (100 * g_leverage * equity - 100 * g_contract_size * MathAbs(distance) * g_leverage * g_volume_of_open_trades_upup - g_leverage * g_margin_stop_out_level * used_margin) / (g_contract_size * (100 * MathAbs(distance) * g_leverage + 100 * current_spread * g_leverage + g_initial_margin_rate * g_margin_stop_out_level));
                break;
            case SYMBOL_CALC_MODE_FOREX_NO_LEVERAGE:
                trade_size = (100 * equity - 100 * g_contract_size * MathAbs(distance) * g_volume_of_open_trades_upup - g_margin_stop_out_level * used_margin) / (g_contract_size * (100 * MathAbs(distance) + 100 * current_spread + g_initial_margin_rate * g_margin_stop_out_level));
                break;
            case SYMBOL_CALC_MODE_CFD:
                trade_size = (100 * equity - 100 * g_contract_size * MathAbs(distance) * g_volume_of_open_trades_upup - g_margin_stop_out_level * used_margin) / (g_contract_size * (100 * MathAbs(distance) + 100 * current_spread + starting_price * g_initial_margin_rate * g_margin_stop_out_level));
                break;
            }
            trade_size = MathFloor(trade_size / g_min_volume) * g_min_volume;

            if(trade_size >= g_min_volume) {
                double opened = OpenUpUp(trade_size, current_ask);
                g_volume_of_open_trades_upup += opened;
                g_cost_of_open_trades_upup += opened * current_ask;
                g_checked_last_open_price = current_ask;
            }
        }
    }
