...
void OpenNewUpWUp() {
    ...
    if(need_new_trade) {
        ...
        case SYMBOL_CALC_MODE_CFDLEVERAGE:
            ...
            break;
        case SYMBOL_CALC_MODE_FOREX:
            trade_size = (100 * g_leverage * equity - 100 * g_contract_size * MathAbs(distance) * g_leverage * g_up_volume_of_open_trades - g_leverage * margin_stop_out_level * used_margin) / (g_contract_size * (100 * MathAbs(distance) * g_leverage + 100 * current_spread * g_leverage + (g_initial_margin > 0 ? g_initial_margin * g_leverage / g_contract_size : 1.0) * g_initial_margin_rate * margin_stop_out_level));
            break;
        case SYMBOL_CALC_MODE_FOREX_NO_LEVERAGE:
            trade_size = (100 * equity - 100 * g_contract_size * MathAbs(distance) * g_up_volume_of_open_trades - margin_stop_out_level * used_margin) / (g_contract_size * (100 * MathAbs(distance) + 100 * current_spread + (g_initial_margin > 0 ? g_initial_margin / g_contract_size : 1.0) * g_initial_margin_rate * margin_stop_out_level));
            break;
        case SYMBOL_CALC_MODE_CFD:
