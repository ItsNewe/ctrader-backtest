...
double g_up_cost_of_open_trades;  // sum(vol_i * open_price_i) for precise loss calc
...
void SizingBuy() {
    ...
    double equity_at_target = equity - g_volume_of_open_trades * MathAbs(distance) * g_contract_size;
    bool need_new_grid = (g_count_buy == 0) ||
...
void OpenNewUpWUp() {
    ...
    // Scan positions without comment (up_w_up) — track both volume and cost basis
    g_up_volume_of_open_trades = 0;
    g_up_cost_of_open_trades = 0;
    for(int i = PositionsTotal() - 1; i >= 0; i--) {
        ulong ticket = PositionGetTicket(i);
        if(ticket == 0) continue;
        ...
        if((PositionGetString(POSITION_SYMBOL) == _Symbol) && (PositionGetInteger(POSITION_TYPE) == POSITION_TYPE_BUY)) {
            ...
            if(k == 0) {  // No comment = up_w_up position
                ...
                g_up_cost_of_open_trades += lots * PositionGetDouble(POSITION_PRICE_OPEN);
            }
        }
    }
    ...
    double equity_after_existing = equity - g_up_volume_of_open_trades * distance * g_contract_size;
    bool need_new_trade = (g_up_volume_of_open_trades == 0) ||
    ...
    if(need_new_trade) {
        ...
        switch(g_calc_mode) {
        case SYMBOL_CALC_MODE_CFDLEVERAGE:
            trade_size = (100 * equity * g_leverage - 100 * g_contract_size * MathAbs(distance) * g_up_volume_of_open_trades * g_leverage - g_leverage * margin_stop_out_level * used_margin) / (g_contract_size * (100 * MathAbs(distance) * g_leverage + 100 * current_spread * g_leverage + starting_price * g_initial_margin_rate * margin_stop_out_level));
            break;
        case SYMBOL_CALC_MODE_FOREX:
            trade_size = (100 * g_leverage * equity - 100 * g_contract_size * MathAbs(distance) * g_leverage * g_up_volume_of_open_trades - g_leverage * margin_stop_out_level * used_margin) / (g_contract_size * (100 * MathAbs(distance) * g_leverage + 100 * current_spread * g_leverage + g_initial_margin_rate * margin_stop_out_level));
            break;
        case SYMBOL_CALC_MODE_FOREX_NO_LEVERAGE:
            trade_size = (100 * equity - 100 * g_contract_size * MathAbs(distance) * g_up_volume_of_open_trades - margin_stop_out_level * used_margin) / (g_contract_size * (100 * MathAbs(distance) + 100 * current_spread + g_initial_margin_rate * margin_stop_out_level));
            break;
        case SYMBOL_CALC_MODE_CFD:
            trade_size = (100 * equity - 100 * g_contract_size * MathAbs(distance) * g_up_volume_of_open_trades - margin_stop_out_level * used_margin) / (g_contract_size * (100 * MathAbs(distance) + 100 * current_spread + starting_price * g_initial_margin_rate * margin_stop_out_level));
            break;
        }
        trade_size = MathFloor(trade_size / g_min_volume) * g_min_volume;

        if(trade_size >= g_min_volume) {
            double opened = OpenUpWUp(trade_size, current_ask);
            g_up_volume_of_open_trades += opened;
            g_up_cost_of_open_trades += opened * current_ask;
            g_up_checked_last_open_price = current_ask;
        }
    }
    ...
    for(int i = PositionsTotal() - 1; i >= 0; i--) {
        ...
        if(PositionGetInteger(POSITION_TYPE) == POSITION_TYPE_BUY) {
            ...
            if(k > 0) {
                ...
                g_up_volume_of_open_trades += lots;
                g_up_cost_of_open_trades += lots * open_price;
