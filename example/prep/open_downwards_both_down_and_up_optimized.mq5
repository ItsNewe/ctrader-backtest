    ...
    if((g_down_volume_of_open_trades == 0) || (g_down_checked_last_open_price > current_bid)) {
        ...
        if(need_new_trade) {
            ...
            case SYMBOL_CALC_MODE_CFDLEVERAGE:
                ...
                break;
            case SYMBOL_CALC_MODE_FOREX:
                trade_size = (100 * g_leverage * equity - 100 * g_contract_size * MathAbs(distance) * g_leverage * g_down_volume_of_open_trades - g_leverage * margin_stop_out_level * used_margin) / (g_contract_size * (100 * MathAbs(distance) * g_leverage + 100 * current_spread * g_leverage + g_initial_margin_rate * margin_stop_out_level));
                break;
            case SYMBOL_CALC_MODE_FOREX_NO_LEVERAGE:
                trade_size = (100 * equity - 100 * g_contract_size * MathAbs(distance) * g_down_volume_of_open_trades - margin_stop_out_level * used_margin) / (g_contract_size * (100 * MathAbs(distance) + 100 * current_spread + g_initial_margin_rate * margin_stop_out_level));
                break;
            case SYMBOL_CALC_MODE_CFD:
