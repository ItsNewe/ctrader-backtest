        ...
        if(PositionGetInteger(POSITION_TYPE) == POSITION_TYPE_BUY) {
            if(PositionsTotal() == 1) {
                if(g_close_all_buy_flag || g_close_all_profitable_buy_flag) {
                    if(open_price + g_current_spread < g_current_ask) {
                        Close(ticket);
                    }
                }
            } else {
...
void SizingBuy() {
    ...
    if(need_new_grid) {
        ...
        if(margin_level > margin_stop_out_level) {
            // Grid loss is triangular: trade i (0-based) enters at starting_price - i*spacing
            // and at end_price loses (len - i*spacing). With uniform spacing = len/(N-1):
            //
            // Sizing=0 (constant lots): each level = trade_size lots
            //   total_loss = C * len * N / 2   =>  avg loss per unit = C * len / 2
            //
            // Sizing=1 (incremental): level i = (i+1)*min_volume lots
            //   total_loss = C * len * N / 3   =>  avg loss per unit = C * len / 3
            //   (bigger positions are at deeper levels with less distance to lose)
            //
            // where C = trade_size * contract_size, N = number of lot-units.
            double C = trade_size * g_contract_size;
            double loss_per_unit;
            if(sizing == 1)
                loss_per_unit = C * len / 3;   // incremental: weighted avg is len/3
            else
                loss_per_unit = C * len / 2;   // constant: uniform avg is len/2
            double d_equity_avg = loss_per_unit + C * g_current_spread;
            ...
            if(used_margin == 0)
                number_of_trades = MathFloor(equity_at_target / (margin_stop_out_level / 100 * local_used_margin + d_equity_avg));
            else
                number_of_trades = MathFloor((equity_at_target - margin_stop_out_level / 100 * used_margin) / (margin_stop_out_level / 100 * local_used_margin + d_equity_avg));
