...
input int Inp_max_number_of_trades  = 200;
...
double OU_cost_of_open_trades_up;    // sum(vol_i * open_price_i) for precise loss calc
double OU_up_checked_last_open_price;
ulong OU_smallest_volume_ticket;
...
int OnInit() {
    ...
    if((Inp_Mult_FU == 0) && (Inp_Mult_OU_Down == 0) && (Inp_Mult_OU_Up == 0))
        return INIT_PARAMETERS_INCORRECT;
    if((Inp_Mult_OU_Down == 0) && ((OU_Sizing != 0) || (OU_Closing_Mode != 0)))
        return INIT_PARAMETERS_INCORRECT;
    if ((Inp_Mult_FU == 0) && (FU_Spacing != 1))
        return INIT_PARAMETERS_INCORRECT;
...
void RefreshMarketData() {
    ...
    if(g_margin_level <= 0)
        g_margin_level = 10000; }

//+------------------------------------------------------------------+
//|                                                                  |
//+------------------------------------------------------------------+
void ScanPositions() {
    double smallest_volume = DBL_MAX;
    int count_UP = 0;
// Reset Aggregators
...
// Single Loop
    ...
    for(int i = total - 1; i >= 0; i--) {
        ...
        if(type == STRAT_FU) {
            FU_pos_count++;
            FU_volume_open += lots;
            if(open_price < FU_lowest_buy)
                FU_lowest_buy = open_price;
            if(open_price > FU_highest_buy)
                FU_highest_buy = open_price;

            // Distance tracking
            if(open_price >= g_ask)
                FU_closest_above = MathMin(FU_closest_above, open_price - g_ask);
            if(open_price <= g_ask)
                FU_closest_below = MathMin(FU_closest_below, g_ask - open_price);
            ...
            if(open_price < OU_lowest_buy)
                OU_lowest_buy = open_price;
            if(open_price > OU_highest_buy)
                OU_highest_buy = open_price;

            if(profit < 0)
                OU_is_there_unprofitable_buy = true;
            if(OU_Sizing == 1)
                OU_trade_size_buy = MathMax(OU_trade_size_buy, lots);

        }
        else if(type == STRAT_OU_UP) {
            OU_volume_of_open_trades_up += lots; // FIXED: Using correct variable name
            OU_cost_of_open_trades_up += lots * open_price;
            double swap = PositionGetDouble(POSITION_SWAP);
            double profit = PositionGetDouble(POSITION_PROFIT) + swap;
            if ((lots < smallest_volume) && (profit > 0)) {
                count_UP++;
                smallest_volume = lots;
                OU_smallest_volume_ticket = ticket; } } }
...
// Stats Update
    ...
    if (count_UP >= Inp_max_number_of_trades) {
        m_trade.PositionClose(OU_smallest_volume_ticket);
        //ScanPositions();
    } }
...
void OU_ProcessClosures() {
    if(!OU_close_all_buy_flag && !OU_close_all_profitable_buy_flag)
        return;
...
// We only loop again if we actually need to close something
    ...
    for(int i = total - 1; i >= 0; i--) {
        ...
        double profit = PositionGetDouble(POSITION_PROFIT) + swap;
        bool close_me = false;
        if(OU_close_all_buy_flag)
            close_me = true;
        else if(OU_close_all_profitable_buy_flag && profit > 0)
            close_me = true;
...
void OU_SizingBuy() {
    ...
    double equity_at_target = g_equity - OU_volume_of_open_trades * MathAbs(distance) * g_contract_size;
    bool need_new_grid = (OU_count_buy == 0) ||
                         (equity_at_target > g_margin_so / 100 * g_margin);
    ...
    if(need_new_grid) {
        ...
        if(margin_level_local > g_margin_so) {
            ...
            double C = g_contract_size * trade_size;
            double loss_per_unit;
            if(OU_Sizing == 1)
                loss_per_unit = C * len / 3;
            else
                loss_per_unit = C * len / 2;
            double d_equity = loss_per_unit + trade_size * g_spread * g_contract_size;
...
void OU_OpenNewUpWUp() {
    ...
    double equity_after_existing = g_equity - OU_volume_of_open_trades_up * distance * g_contract_size;
    bool need_new_trade = (OU_volume_of_open_trades_up == 0) ||
                          (equity_after_existing > g_margin_so / 100 * g_margin);
    ...
    if(need_new_trade) {
        ...
        if(trade_size >= g_min_volume) {
            if(OU_SendOrder(trade_size, false)) {
                OU_cost_of_open_trades_up += trade_size * g_ask;
                OU_up_checked_last_open_price = g_ask; } } } }
...
//+------------------------------------------------------------------+
bool FU_OpenBuy() {
    if(FU_trade_size_buy < g_min_volume)
        return false;
...
void FU_CalcLotSize() {
    ...
    if(g_margin > 0 && (eq_at_target / g_margin * 100) <= g_margin_so)
        return;

    double unit = g_min_volume;
    double d_equity = g_contract_size * unit * FU_spacing_val * (num_trades * (num_trades + 1) / 2);
