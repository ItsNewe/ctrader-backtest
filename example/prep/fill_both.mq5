...
// Target Price: Price Drops X%
    ...
    // BUG FIX: was missing g_spacing multiplier. Each grid level is spaced
    // by g_spacing, so level i loses i*spacing. Sum = spacing * N*(N+1)/2.
    double d_equity = g_contract_size * unit * g_spacing * (num_trades * (num_trades + 1) / 2);
    double spread = g_point * SymbolInfoInteger(_Symbol, SYMBOL_SPREAD);
