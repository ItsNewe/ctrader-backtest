"""
Download XAUUSD tick data for 2025 from MT5 ()
Saves in tab-separated format compatible with C++ backtest engine
"""
...
def download_ticks():
    ...
    output_file = "validation/Grid/XAUUSD_TICKS_2025.csv"
    output_df.to_csv(output_file, sep='\t', index=False)
    print(f"\nSaved to: {output_file}")

    # Also save a backup with timestamp
    backup_file = f"validation/Grid/XAUUSD_TICKS_2025_downloaded_{datetime.now().strftime('%Y%m%d_%H%M%S')}.csv"
    output_df.to_csv(backup_file, sep='\t', index=False)
...
if __name__ == "__main__":
    print("=" * 60)
    print("XAUUSD Tick Data Downloader - 2025")
    print("=" * 60)
