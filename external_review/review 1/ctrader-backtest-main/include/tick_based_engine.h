...
#define ENGINE_LOG_INFO(msg) std::cout << "[ENGINE-INFO] " << msg << std::endl
#define ENGINE_LOG_WARN(msg) std::cout << "[ENGINE-WARN] " << msg << std::endl
#define ENGINE_LOG_ERROR(msg) std::cerr << "[ENGINE-ERROR] " << msg << std::endl
...
public:
    explicit TickBasedEngine(const TickBacktestConfig& config)
        ...
        if (config.contract_size <= 0) {
            throw EngineException("Contract size must be greater than zero");
        }
        if (config.leverage <= 0) {
            throw EngineException("Leverage must be greater than zero");
        }
    }
     ...
     */
    std::shared_ptr<Trade> OpenMarketOrder(const std::string& direction, double lot_size,
        ...
        if (!current_tick_.timestamp.empty()) {
            ...
            auto trade = CreateTrade(direction, entry_price, lot_size, stop_loss, take_profit);
     ...
     */
    bool ClosePosition(std::shared_ptr<Trade> trade, const std::string& reason = "Manual") {
    ...
    const std::vector<std::shared_ptr<Trade>>& GetOpenPositions() const { return open_positions_; }
...
private:
    ...
    Tick current_tick_;
    std::vector<std::shared_ptr<Trade>> open_positions_;
    ...
    double GetPipValue() const {
        // Dynamic pip calculation based on symbol
        if (config_.symbol.find("JPY") != std::string::npos) {
            return 0.01;
        }
        if (config_.symbol.find("XAU") != std::string::npos || config_.symbol.find("GOLD") != std::string::npos) {
            return 0.01;
        }
        return 0.00001;
    }

    std::shared_ptr<Trade> CreateTrade(const std::string& direction, double entry_price,
    ...
    void UpdateEquity(const Tick& tick) {
        ...
        for (const auto& trade : open_positions_) {
        ...
        // Update drawdown based on equity
        if (equity_ > peak_balance_) {
            peak_balance_ = equity_;
        }
        double current_drawdown = peak_balance_ - equity_;
    ...
    void CheckMarginStopOut(const Tick& tick) {
        ...
        if (margin_level < STOP_OUT_LEVEL) {
            std::stringstream ss;
            ss << "MARGIN STOP OUT: Level=" << margin_level << "%, Equity=$" << equity_ << ", Used Margin=$" << used_margin;
    ...
    void ProcessOpenPositions(const Tick& tick) {
        ...
        std::vector<std::shared_ptr<Trade>> positions_to_close;
        ...
        for (const auto& trade : positions_to_close) {
