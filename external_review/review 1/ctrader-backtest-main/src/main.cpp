...
int main(int argc, char* argv[]) {
  ...
  if (!cli_params.json_output) {
    std::cout << "=== cTrader Backtest Engine ===" << std::endl;
  }
  ...
  try {
    ...
    for (int i = 0; i < 500; ++i) {
      Bar bar(1609459200 + i * 3600,
              1.1000 + i * 0.0001,
              1.1000 + i * 0.0001 + 0.0005,
              1.1000 + i * 0.0001 - 0.0003,
              1.1000 + i * 0.0001 + 0.0002,
              1000 + rand() % 500,
              100 + rand() % 50);
      bars.push_back(bar);
    }
    ...
    if (!cli_params.json_output) {
      std::cout << "Running demonstration backtest with " << bars.size() << " bars..." << std::endl;
    }
