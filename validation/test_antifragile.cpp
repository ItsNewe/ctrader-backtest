...
int main(int argc, char* argv[]) {
    if (argc < 2) {
        std::cerr << "Usage: test_antifragile <asset> [base_lot] [spacing] [exponent] [tp_pct]\n";
        return 1;
    }
    ...
    if (argc >= 6) cfg.take_profit_pct = std::atof(argv[5]);
    ...
    std::cout << "Take Profit %: " << cfg.take_profit_pct << "\n";
    std::cout << "---\n";
