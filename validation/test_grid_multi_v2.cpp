/**
 * Multi-Instrument Grid Test V2
 * Matches MQ5 grid_multi_symbol.mq5 logic exactly
 * Uses proper margin-based lot sizing with cross-symbol awareness
 */
...
struct SymbolState {
    ...
    double contract_size;
    double leverage;
    double survive_down;  // percentage
...
public:
    ...
    MultiSymbolEngine(double init_bal, double au_survive, double ag_survive) {
        ...
        gold.initial_margin_rate = 1.0;  // 100% margin rate
        gold.min_volume = 0.01;
        gold.max_volume = 100.0;
        gold.volume_of_open_trades = 0;
        gold.checked_last_open_price = 0;
        ...
        silver.checked_last_open_price = 0;
    }
    ...
    double GetUsedMargin(double au_price, double ag_price) {
        double margin = 0;
        // SYMBOL_CALC_MODE_CFDLEVERAGE: margin = lots * contract * price / leverage
        for (const auto& p : gold.positions) {
            margin += p.lot_size * gold.contract_size * au_price / gold.leverage;
        }
        for (const auto& p : silver.positions) {
            margin += p.lot_size * silver.contract_size * ag_price / silver.leverage;
        }
        return margin;
    }

    double GetMarginLevel(double equity, double used_margin) {
        if (used_margin <= 0) return 10000.0;  // No positions = infinite margin level
        return (equity / used_margin) * 100.0;
    }

    // Match MQ5 formula exactly for SYMBOL_CALC_MODE_CFDLEVERAGE
    double CalculateTradeSize(SymbolState& sym, double current_ask, double equity,
                              double used_margin, double current_margin_level) {
        double distance = current_ask * sym.survive_down / 100.0;
        double spread_and_commission = 0;  // Simplified

        // MQ5 formula for CFDLEVERAGE:
        // trade_size = (100*equity*leverage - 100*contract*distance*volume*leverage - leverage*stop_out*used_margin)
    ...
    bool ShouldOpenTrade(SymbolState& sym, double current_ask, double equity,
                         double used_margin, double current_margin_level) {
        if (sym.volume_of_open_trades == 0) return true;

        // Check if price dropped enough that we shouldn't add
        ...
        double end_price = current_ask * (1.0 - sym.survive_down / 100.0);

        return (current_ask - price_difference) < end_price;
    }
    ...
    void ProcessSymbol(const std::string& symbol, double ask, double bid,
        ...
        double au_price = (symbol == "XAUUSD") ? ask : other_ask;
        double ag_price = (symbol == "XAGUSD") ? ask : other_ask;
        ...
        double margin_level = GetMarginLevel(equity, used_margin);
        ...
        // "Up while up" - only open on new high
        if (sym.volume_of_open_trades == 0 || ask > sym.checked_last_open_price) {
            sym.checked_last_open_price = ask;

            // Update volume from positions
            sym.volume_of_open_trades = 0;
            for (const auto& p : sym.positions) {
                sym.volume_of_open_trades += p.lot_size;
            }

            if (ShouldOpenTrade(sym, ask, equity, used_margin, margin_level)) {
                double trade_size = CalculateTradeSize(sym, ask, equity, used_margin, margin_level);
...
TestResult RunTest(double au_survive, double ag_survive) {
    ...
    if (!engine.stop_out) {
        ...
        result.au_volume = engine.gold.volume_of_open_trades;
        result.ag_volume = engine.silver.volume_of_open_trades;
    } else {
...
int main() {
    std::cout << "=== Multi-Symbol Grid V2 (MQ5 Match) ===" << std::endl;
    ...
    std::cout << "AU%   AG%   Return     DD%      AU_pos  AG_pos  AU_vol  AG_vol" << std::endl;
    std::cout << "----  ----  ---------  -------  ------  ------  ------  ------" << std::endl;
    ...
    for (int i = 0; i < 20 && i < (int)results.size(); i++) {
        ...
        if (r.stop_out) {
            ...
            std::cout << std::setw(4) << r.au_survive << "  "
                      ...
                      << std::setw(6) << r.ag_entries << "  "
                      << std::setw(6) << r.au_volume << "  "
                      << std::setw(6) << r.ag_volume << std::endl;
        }
    }
    ...
    if (!results.empty() && !results[0].stop_out) {
        ...
        std::cout << "Return: " << best.ret << "x, DD: " << best.max_dd_pct << "%" << std::endl;
        std::cout << "Positions: AU=" << best.au_entries << " (" << best.au_volume << " lots), "
                  << "AG=" << best.ag_entries << " (" << best.ag_volume << " lots)" << std::endl;
    }
