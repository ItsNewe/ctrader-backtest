/**
 * V7 Parameter Sweep Test - EURUSD
 *
 * Tests V7 volatility filter strategy on EURUSD forex pair.
 * Adjustments from XAUUSD version:
 * - Spacing: 0.001 (10 pips) instead of 1.0 (EURUSD moves in smaller increments)
 * - Contract size: 100000 (standard forex lot) instead of 100
 * - ATR-based volatility filter logic remains the same
 */
...
Result RunTest(const std::vector<Tick>& ticks, const V7Config& cfg) {
    ...
    double contract_size = 100000.0;  // Standard forex lot (100,000 units)
    double leverage = 500.0;
    double spacing = 0.001;  // 10 pips for EURUSD (0.0001 * 10)
    ...
    for (const Tick& tick : ticks) {
        ...
        if (dd_pct < cfg.stop_new_at_dd && volatility_ok && (int)positions.size() < cfg.max_positions) {
            ...
            if (should_open) {
                double lot = 0.01;  // Micro lot for forex
...
// Generate synthetic EURUSD tick data for testing
std::vector<Tick> GenerateSyntheticTicks(size_t count, double start_price = 1.1000, double spread = 0.0003) {
    std::vector<Tick> ticks;
    ticks.reserve(count);

    double price = start_price;
    double volatility = 0.0001;  // Base volatility for forex

    for (size_t i = 0; i < count; i++) {
        // Create varying volatility periods
        if (i % 10000 < 3000) {
            volatility = 0.00005;  // Low volatility period
        } else if (i % 10000 < 6000) {
            volatility = 0.00015;  // Medium volatility
        } else {
            volatility = 0.00025;  // High volatility
        }
...
int main() {
    std::cout << "================================================================" << std::endl;
    std::cout << "     V7 PARAMETER SWEEP - EURUSD VOLATILITY FILTER TEST" << std::endl;
    std::cout << "================================================================" << std::endl;
    std::cout << std::endl;

    // Try to load real EURUSD tick data
    std::string filename = "EURUSD_TICKS_202401.csv";
    std::cout << "Loading EURUSD tick data from: " << filename << std::endl;

    std::vector<Tick> ticks = LoadTicks(filename, 500000);  // 500K ticks for better accuracy

    if (ticks.empty()) {
        std::cout << "File load failed. Using synthetic EURUSD data instead..." << std::endl;
        srand(42);
    ...
    std::cout << "Loaded " << ticks.size() << " ticks" << std::endl;
    std::cout << "Price range: " << std::fixed << std::setprecision(5)
              << ticks.front().bid << " -> " << ticks.back().bid << std::endl;
    std::cout << "Avg spread: " << std::setprecision(5)
              << (ticks.front().spread() + ticks.back().spread()) / 2.0 << std::endl;
    std::cout << std::endl;

    // EURUSD-specific parameter ranges
    // Adjusted threshold values since forex volatility patterns differ from gold
    ...
    std::vector<double> tp_mult_vals = {1.0, 1.5, 2.0, 2.5};  // Added 2.5 for forex
    ...
    std::cout << "EURUSD Config: spacing=0.001 (10 pips), contract_size=100000" << std::endl;
    std::cout << std::endl;
    ...
    std::cout << "RECOMMENDED PARAMETERS FOR EURUSD:" << std::endl;
    ...
    // Compare with XAUUSD default V7 params applied to EURUSD
    std::cout << "COMPARISON WITH DEFAULT V7 PARAMS (100/500/0.8/1.0):" << std::endl;
    ...
    std::cout << "  Default V7:  Return=" << default_result.return_pct << "%, MaxDD="
    ...
    std::cout << std::endl;
    std::cout << "================================================================" << std::endl;
    std::cout << "                    EURUSD V7 ANALYSIS COMPLETE" << std::endl;
    std::cout << "================================================================" << std::endl;

    // Final verdict
    std::cout << std::endl;
    if (profitable > all_results.size() / 2 && best.result.return_pct > 0) {
        std::cout << "VERDICT: V7 volatility filter strategy WORKS on EURUSD!" << std::endl;
        std::cout << "  " << profitable << "/" << all_results.size() << " configurations were profitable." << std::endl;
    } else if (profitable > 0) {
        std::cout << "VERDICT: V7 strategy shows MIXED results on EURUSD." << std::endl;
        std::cout << "  Only " << profitable << "/" << all_results.size() << " configurations were profitable." << std::endl;
        std::cout << "  Careful parameter tuning required." << std::endl;
    } else {
        std::cout << "VERDICT: V7 strategy does NOT work well on EURUSD." << std::endl;
        std::cout << "  No profitable configurations found." << std::endl;
    }
