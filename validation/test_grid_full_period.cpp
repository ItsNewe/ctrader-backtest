/**
 * Multi-Instrument Grid Test - Full Period
 * Loads 2025 + Jan 2026 tick data to match MT5 test period
...
size_t LoadTickFile(const std::string& path, const std::string& symbol, bool has_last_column) {
    ...
    while (std::getline(file, line)) {
        ...
        std::string bid_str, ask_str, extra;
...
void LoadAllTicks() {
    ...
    // Load XAUUSD 2025
    std::cout << "Loading XAUUSD 2025..." << std::endl;
    size_t au_2025 = LoadTickFile(
        "C:\\Users\\user\\Documents\\ctrader-backtest\\validation\\Grid\\XAUUSD_TICKS_2025.csv",
        "XAUUSD", true);
    std::cout << "  " << au_2025 << " ticks" << std::endl;

    // Load XAUUSD Jan 2026
    std::cout << "Loading XAUUSD Jan 2026..." << std::endl;
    size_t au_2026 = LoadTickFile(
        "C:\\Users\\user\\Documents\\ctrader-backtest\\validation\\Grid\\XAUUSD_TICKS_JAN2026.csv",
        "XAUUSD", true);
    std::cout << "  " << au_2026 << " ticks" << std::endl;

    // Load XAGUSD 2025
    std::cout << "Loading XAGUSD 2025..." << std::endl;
    size_t ag_2025 = LoadTickFile(
        "C:\\Users\\user\\Documents\\ctrader-backtest\\validation\\XAGUSD\\XAGUSD_TICKS_2025.csv",
        "XAGUSD", false);
    std::cout << "  " << ag_2025 << " ticks" << std::endl;

    // Load XAGUSD Jan 2026
    std::cout << "Loading XAGUSD Jan 2026..." << std::endl;
    size_t ag_2026 = LoadTickFile(
        "C:\\Users\\user\\Documents\\ctrader-backtest\\validation\\XAGUSD\\XAGUSD_TICKS_JAN2026.csv",
        "XAGUSD", false);
    std::cout << "  " << ag_2026 << " ticks" << std::endl;
...
TestResult RunTest(double au_survive, double ag_survive) {
    ...
    int total_pos = engine.gold.positions.size() + engine.silver.positions.size();
    ...
    if (!engine.stop_out) {
        ...
        result.total_trades = total_pos + engine.positions_closed;
...
int main() {
    std::cout << "=== Multi-Symbol Grid Full Period ===" << std::endl;
    ...
    // Parameter ranges matching MT5 optimization
    ...
    std::cout << "AU%   AG%   Return      DD%      Trades  Closed" << std::endl;
    std::cout << "----  ----  ----------  -------  ------  ------" << std::endl;
    ...
    for (int i = 0; i < 20 && i < (int)results.size(); i++) {
        ...
        if (r.stop_out) {
            std::cout << std::setw(4) << r.au_survive << "  "
                      << std::setw(4) << r.ag_survive << "  STOP-OUT" << std::endl;
        } else {
            std::cout << std::setw(4) << r.au_survive << "  "
                      ...
                      << std::setw(6) << r.positions_closed << std::endl;
        }
    }
    ...
    if (!results.empty() && !results[0].stop_out) {
        ...
        std::cout << "Return: " << best.ret << "x ($" << std::fixed << std::setprecision(0)
                  << (best.ret * 10000) << ")" << std::endl;
        std::cout << "Max DD: " << std::setprecision(2) << best.max_dd_pct << "%" << std::endl;
        std::cout << "Total trades: " << best.total_trades << std::endl;
    }
