...
int main() {
    std::cout << "========================================================" << std::endl;
    std::cout << " SILVER OPTIMIZATION #2: Lookback x Spacing sweep" << std::endl;
    std::cout << " Survive=19% and 30%, Balance=$10000" << std::endl;
    std::cout << "========================================================" << std::endl;
    ...
    std::vector<double> survive_vals = {19, 30};
    std::vector<double> spacing_vals = {0.05, 0.10, 0.15, 0.20, 0.30, 0.50, 0.75, 1.00, 1.50};
    std::vector<double> lookback_vals = {0.5, 1.0, 2.0, 4.0, 8.0, 12.0};

    for (double survive : survive_vals) {
        std::cout << "\n============ Survive=" << (int)survive << "% ============" << std::endl;
        std::cout << std::setw(8) << "Space"
                  << std::setw(6) << "LB"
                  << std::setw(12) << "FinalEq"
                  << std::setw(8) << "Return"
                  << std::setw(8) << "MaxDD%"
        ...
        for (double spacing : spacing_vals) {
            for (double lookback : lookback_vals) {
                ...
                std::cout << std::setw(7) << std::fixed << std::setprecision(2) << spacing << "$"
                          << std::setw(5) << std::setprecision(1) << lookback << "h"
