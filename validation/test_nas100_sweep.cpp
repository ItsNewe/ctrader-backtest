...
TestResult run_backtest(const char* filename, double survive, double spacing,
...
int main() {
    ...
    for (double survive : {4.0, 8.0, 10.0, 15.0, 20.0, 25.0, 30.0, 40.0, 50.0}) {
        auto r = run_backtest(nas_file, survive, 50.0, false, 0, 1.0, 1.0);
    ...
    for (double spacing : {5.0, 10.0, 25.0, 50.0, 100.0, 200.0, 500.0}) {
        auto r = run_backtest(nas_file, 30.0, spacing, false, 0, 1.0, 1.0);
    ...
    for (double atr : {1.5, 2.0, 2.5, 3.0, 4.0, 5.0, 10.0}) {
        auto r = run_backtest(nas_file, 30.0, 50.0, true, atr, 1.0, 1.0);
    ...
    for (double survive : {4.0, 8.0, 10.0, 15.0, 20.0}) {
        auto r = run_backtest(nas_file, survive, 50.0, true, 2.0, 1.0, 1.0);
        std::cout << std::setw(6) << survive << "%   "
                  << "$" << std::setw(9) << r.final_equity << "  "
                  << std::setw(5) << (r.final_equity / 10000.0) << "x  "
                  << std::setw(5) << r.max_dd << "%   "
