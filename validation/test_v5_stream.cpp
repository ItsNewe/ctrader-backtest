...
int main() {
    ...
    for (const auto& start : starts) {
        ...
        for (const auto& cfg : configs) {
            ...
            Result r = RunTest(cfg.name, cfg.survive, cfg.spacing, cfg.ma, start);
            results.push_back(r);
            double ret_dd = (r.max_dd > 0) ? (r.ret / (r.max_dd / 100.0)) : 0;
            std::cout << " " << r.ret << "x | " << r.max_dd << "% DD | Ret/DD: " << ret_dd
    ...
    std::cout << std::endl;
    std::cout << "CombinedJu (UNIFORM, from earlier tests):" << std::endl;
