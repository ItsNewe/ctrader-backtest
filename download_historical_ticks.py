"""
Download historical XAUUSD tick data from MT5 ( or Grid)
Supports downloading multiple years for out-of-sample testing

Usage:
    python download_historical_ticks.py 2024           # Download 2024 data
    python download_historical_ticks.py 2023 2024     # Download 2023 and 2024
    python download_historical_ticks.py 2020 2024     # Download 2020-2024 range
"""
...
def download_year(symbol, year, output_dir):
    ...
    while current_start < end_date:
        ...
        if ticks is not None and len(ticks) > 0:
            ...
            if error[0] == -2:  # No data available
    ...
    # Create output in MT5 tick export format
...
def main():
    ...
    print("="*60)
    print("Historical Tick Data Downloader")
    ...
    # Get terminal info to determine broker
    terminal_info = mt5.terminal_info()
    if terminal_info:
        print(f"\nConnected to: {terminal_info.name}")
        print(f"Company: {terminal_info.company}")

        # Determine output directory based on broker
        if "Broker" in terminal_info.company:
            output_dir = "validation/Broker/historical"
            broker = "Broker"
        elif "Grid" in terminal_info.company:
            output_dir = "validation/Grid/historical"
            broker = "Grid"
        else:
    ...
    if all(r[0] for r in results.values()):
        ...
        print("  This may be normal if the broker doesn't have historical data for those years.")
