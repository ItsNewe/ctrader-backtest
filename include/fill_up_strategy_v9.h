#ifndef FILL_UP_STRATEGY_V9_H
#define FILL_UP_STRATEGY_V9_H
...
/**
 * Fill-Up Grid Trading Strategy V9 - ULTIMATE OPTIMIZED VERSION
 *
 * This version combines ALL optimization findings from V1-V8 sweeps:
 *
 * Key features:
 * 1. Optimal spacing: 0.75 (from parameter sweep - better than 1.0)
 * 2. Session filter: Avoid US session peak (14-18 UTC) by default
 * 3. Anti-martingale option: +25% lot after each consecutive win
 * 4. DD-based lot scaling: Reduce lots as drawdown increases (from V8)
 * 5. Optimized ATR parameters: 50/1000/0.6 (from V7/V8 sweep)
 * 6. Wider TP: 2.0x multiplier for better risk/reward
 * 7. Tighter protection levels: 3%/5%/15% (from V8)
 *
 * Risk philosophy:
 * - Aggressive early intervention to prevent deep drawdowns
 * - Reduce position size when already in drawdown
 * - Increase position size after wins (anti-martingale, optional)
 * - Avoid high-volatility sessions (US peak hours)
 * - Strict volatility filter (only trade in calm markets)
 * - Wider TP to improve win rate and reduce churn
 *
 * This is the "ultimate" version designed for maximum risk-adjusted returns.
 */
class FillUpStrategyV9 {
public:
    struct Config {
    ...
    explicit FillUpStrategyV9(const Config& config)
        : config_(config),
          lowest_buy_(DBL_MAX),
    ...
    // Get effective lot size at current DD (including anti-martingale)
    double GetEffectiveLotSize(double dd_pct) const {
        ...
        double lot = base_lot * dd_scale * am_scale;
...
private:
    ...
    void OpenNew(TickBasedEngine& engine, double current_dd_pct) {
        ...
        // Apply both scales
        double lot_size = NormalizeVolume(config_.min_volume * dd_scale * am_scale);
